...
where \(U\) is the energy density (measured in \si{\erg\per\cubic\centi\metre}), \(n_b\) is the baryon density (measured in \si{\per\cubic\centi\metre}), while \(\Gamma\) and \(\Lambda\) are the heating and cooling functions, both measured in \si{\erg\cubic\centi\metre\per\second}.\footnote{See \url{http://lss.fnal.gov/archive/2012/pub/fermilab-pub-12-107-a.pdf}}
...
Now, let us consider all the space-like (unit) vectors \(n^\mu\), which have \(n_\mu u^\mu = 0\) and \(n^\mu n_\mu = 1\). They span a three-dimensional sphere.

If we have a function \(F\colon S^2 \rightarrow \mathbb R\), we can decompose it into harmonics as such:

\begin{equation}
    F(n) = \sum _{k=0}   ^{\infty}
    F_{\alpha_1 \dots \alpha_k} \prod_{i=0}^k n^{\alpha_i} 
\end{equation}
