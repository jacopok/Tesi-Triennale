...
The only interesting component of this is the radial one, so we need to calculate \(a^1 = u^\mu \nabla_\mu u^1 = \dv*{u^1}{\tau} + \Gamma^1_{\mu \nu} u^\mu u^\nu \). To do this we will need the Schwarzschild Christoffel coefficients:

\begin{equation}
  \Gamma^1_{\mu \nu} = \left[\begin{matrix}- \frac{2 M \left(\frac{M}{r} - \frac{1}{2}\right)}{r^{2}} & 0 & 0 & 0\\0 & \frac{2 M \left(\frac{M}{r} - \frac{1}{2}\right)}{r^{2} \left(- \frac{2 M}{r} + 1\right)^{2}} & 0 & 0\\0 & 0 & 2 r \left(\frac{M}{r} - \frac{1}{2}\right) & 0\\0 & 0 & 0 & 2 r \left(\frac{M}{r} - \frac{1}{2}\right) \sin^{2}{\left(\theta \right)}\end{matrix}\right]
\end{equation}
