...
they are  represented in 4D spacetime as \(x^\mu \rightarrow \Lambda^\mu\,_\alpha x^\alpha + a^\mu\), with the \(\Lambda_\mu\,^\alpha\) being \((1,1)\) tensors which satisfy \(\Lambda^\mu\,_\alpha \Lambda^\nu\,_\beta \eta_{\mu\nu} = \eta_{\alpha \beta}\), and \(a^\mu\) being a generic constant 4-vector.
...
Suppose we have a curve \(\xi^i(\tau)\) in \(\Sigma\). Then we can look at the two-dimensional surface defined by the evolution of \(\xi^i(\tau)\): \(x^{\mu} (\xi^i(\tau), s) \defeq x^\mu (\tau, s)\). If we define the ``spatial'' tangent vector \(\lambda^\mu = \dv*{x^\mu}{\tau} \), it follows from Schwarz's theorem that:
...
The \emph{Einstein Equivalence Principle} states \cite[100]{Carroll:1997ar} that in small enough regions of spacetime the laws of physics are those of special relativity, and we cannot detect gravitational effect locally. The frame of reference in which we must write the laws to put them in special-relativistic form is called the Locally Inertial Reference Frame or Local Rest Frame.
...
The constant comes by imposing continuity with the newtonian limit, in which we know the gravitational field \(\Phi\) is determined by the matter density \(\rho_0\) according to the Poisson equation \(\nabla^2 \Phi = 4 \pi \rho_0\), where the classical \(\rho_0\) is substituted in the relativistic formulation by \(T_{00}\) while \(\Phi\) is substituted by \(h_{00}\), with \(g_{00} = - 1 + h_{00}\) (see \cite[eq. 4.46]{Carroll:1997ar}).
...
If we wanted to analyze the accretion problem without \emph{any} approximation we would need to consider the stress-energy tensor of the fluid when solving the Einstein Field Equations; however this would make the already hard problem completely intractable and it is reasonable to assume that the gas falling into a black hole is relatively light compared to the black hole itself, therefore we analyze the accretion problem with a fixed Schwarzschild metric.
