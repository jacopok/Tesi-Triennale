...
Once we have this, we can define the 4-acceleration \(a^\mu = \dv*{u^\mu}{\tau} \) and the 4-momentum \(p^\mu = m u^\mu\), where \(m\) is the rest mass of the body at hand.
...
We can see that it approaches the flat metric $\eta_{\mu\nu} = \diag\qty(-, +, +, +)$ both in the limit $M\rightarrow 0$ and the limit \(r \rightarrow \infty\). Its determinant is $g = -r^4 \sin^2 \theta$.
