...
Following \textcite[]{ThorneFLammmangZytkow:1981feb}. We are working with the following assumptions:
...
\begin{subequations} \label{eq:NTZ91-moment-equations}
\begin{align}
  y w_1^{\prime} + 2 \qty(y^{\prime} + \frac{y}{r}) w_1
  -yv w_0^{\prime}
...
Equations \eqref{eq:NTZ91-moment-equations} are the ones which appear in  \textcite[eq. 4]{NobiliTurollaZampieri:1991dec}. 
...
\paragraph{The transfer equations (? to rename)}
...
where \(\rho_0\) is the rest mass density, \(w_k\) are the PSTF scalar moments, \(\varepsilon_k\) is the emissivity and \(\kappa_k\) is the opacity of the gas.
...
The Bernoulli equation can be written in the LRF as \(\nabla_\mu T^{t\mu} = 0\).
Then, applying \eqref{eq:covariant-divergence} and our symmetry assumptions:

\begin{equation}
  \nabla_\mu T^{t \mu}
  = \frac{1}{r^2 \sin \theta} \pdv{}{r} \qty(r^2 \sin\theta T^{tr})
  = \frac{1}{r^2} \pdv{}{r} \qty(r^2 T^{tr}) = 0
  \implies
  r^2 T^{tr} = \const
\end{equation}

\paragraph{The moment equations with only radial derivatives}
...
\begin{align}
  ...
  -\frac{4}{3} v w_{0}\left[\frac{(v y)^{\prime}}{v y}+2\right]&=\frac{r R_{g} s_{0}}{y} \\
  w_{2}^{\prime}-v w_{1}^{\prime}+\frac{1}{3} w_{0}^{\prime} +w_{2}\left(3+\frac{y^{\prime}}{y}\right)-2 v w_{1}\left[\frac{(v y)^{\prime}}{v y}+1\right]+\frac{4}{3} \frac{y^{\prime}}{y} w_{0} &=\frac{r R_{g} s_{1}}{y}
\end{align}
\end{subequations}
